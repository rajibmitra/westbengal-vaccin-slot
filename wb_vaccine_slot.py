...
def getWbDistrictID(st_id, lookout_district):
    """
    ...
    if all(key in settings for key in key_list):
        ...
        if("pincode" in settings.keys()):
            ...
            if(state!="" and dist!=""):
                ...
                st_id = westBengalStateID(state)
                print("State ID: ", st_id)
                if(st_id!=""):
                    DISTRICT_ID = getWbDistrictID(st_id,dist)
        ...
        print("Arguments Missing/Invalid. Check configs.json")
            
    print(westBengalStateID("westbengal"))
    print(getWbDistrictID(36,"birbhum"))
